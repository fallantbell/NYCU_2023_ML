...
def train(x, cond, modules, optimizer, kl_anneal, args):
    ...
    '''
        x.shape = [12,30,3,64,64] => [batch,time,channel,height,width]
    '''
    x = x.transpose(0,1)
    ...
    for i in range(1, args.n_past + args.n_future):
        if use_teacher_forcing: # x_{t-1} = GT
            xt_last = x[i-1]
        else:
            xt_last = x_pred[i-1] # x_{t-1} = predicted
        
        ht,_ = modules['encoder'](x[i])

        if i<args.n_past:
            ht_last,skip = modules['encoder'](xt_last) # skip 用來輔助decoder還原影像
        else:
            ht_last,_ = modules['encoder'](xt_last)
            
        z,mu,logvar = modules['posterior'](ht) # z mu logvar shape = [12,64] [batch,z_t]

        gt = modules['frame_predictor'](torch.cat([cond[i],ht_last,z],1))

        predict = modules['decoder']([gt,skip])

        x_pred.append(predict)

        mse += criterion(predict,x[i])
...
class kl_annealing():
    def __init__(self, args):
        ...
        self.if_cyclical = args.kl_anneal_cyclical
        self.ratio = args.kl_anneal_ratio
        self.cycle = args.kl_anneal_cycle
        self.epoch = args.niter*args.epoch_size

        self.beta = np.ones(self.epoch)

        name = "cyclical"
        if self.if_cyclical == False: # monotonic
            name = "monotonic"
            self.cycle = 1
        
        epoch_per_cycle = self.epoch/self.cycle
        linear_cycle = epoch_per_cycle*self.ratio

        num = 0
        linear_value = np.linspace(0,1,num=int(linear_cycle))
        for i in range(self.cycle):
            num = int(i*epoch_per_cycle) 
            for j in range(int(linear_cycle)):
                self.beta[num]=linear_value[j]
                num+=1
    ...
    def get_beta(self):
        self.update()
        return self.beta[self.now_epoch]
'''
...
def test(args,device):
    saved_model = torch.load('logs/fp/Cyclical_tfr/model.pth')
    ...
    for _ in tqdm(range(len(test_data) // args.batch_size)):
        ...
        _, _, psnr = finn_eval_seq(pred_seq[args.n_past:args.n_past+args.n_future], test_seq[args.n_past:args.n_past+args.n_future])
    ...
    for epoch in range(start_epoch, start_epoch + niter):
        ...
        if epoch >= args.tfr_start_decay_epoch:
            ...
            if args.tfr_decay_step == 0:
                args.tfr = max(1-(epoch-args.tfr_start_decay_epoch)/(niter-args.tfr_start_decay_epoch),0)
            else:
