...
public:
	...
	virtual float update(const board& b, float u) {
		...
		// 更新這個pattern 的weight
		// 因為權重是由所有旋轉/鏡像加總得來的，所以u也要除以 iso_last
		u = u/iso_last;
		float value = 0.0;
...
public:
	...
	float estimate(const board& b) const {
		...
		float value = 0;
		// n tuple 的模板，預設四個，將每個模板的旋轉/鏡像加總得分，代表這個board玩到遊戲結束的預期分數
	...
	float update(const board& b, float u) const {
		...
		// u 是要更新的權重
		// 因為權重是由所有pattern 加總而來，所以更新時 u 要除以pattern 數量
	...
	state select_best_move(const board& b) const {
		...
		for (state* move = after; move != after + 4; move++) {
			if (move->assign(b)) {
				// TODO
				debug << "select best move " << std::endl;

				// 執行動作後得到的reward
				float r = move->reward();

				// 移動後對空白的區塊 pop 2(90%) 4(10%)
				// 對所有可能情況計算平均得分
				int blank=0;
				...
				// 移動之後的期望得分
				total_value = r+total_value/blank;
				move->set_value(total_value);
